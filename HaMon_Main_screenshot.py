...
# --- 共通クラス (Wave, Player, AttackerHuman, AttackerAuto は前回と同じ) ---
...
class App:
    def __init__(self):
        pyxel.init(160, 120, title="HaMon - Press P to Pause")
        self.state = "MENU"
    ...
    def reset(self, mode):
        ...
        self.paused = False # リセット時はポーズ解除

    def update(self):
        # Pキーで一時停止をトグル (どの状態でも可能)
        if pyxel.btnp(pyxel.KEY_P):
            self.paused = not self.paused

        # 一時停止中は以下の更新ロジックをすべてスキップ
        if self.paused:
            return
    ...
    def draw(self):
        ...
        if self.state == "MENU":
            ...
            if self.player.still_frames > 0 and self.game_state == "PLAYING":
        ...
        # 一時停止中の表示 (最前面)
        # if self.paused:
        #     # 画面を少し暗くする演出（簡易版：テキストで明示）
        #     pyxel.rect(60, 50, 40, 12, 0)
        #     pyxel.rectb(60, 50, 40, 12, 7)
        #     pyxel.text(70, 54, "PAUSE", pyxel.frame_count % 16)
